package codes.laivy.jhttp.pseudo.provided;
...
public final class PseudoEncoding implements PseudoString<Encoding> {
    ...
    private PseudoEncoding(@NotNull String string) {
        this.pseudo = PseudoString.create(string, () -> Encoding.contains(string), () -> Encoding.retrieve(string).orElseThrow(() -> new NullPointerException("there's no encoding with name '" + string + "' registered")));
    }
